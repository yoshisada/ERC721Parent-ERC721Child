...
export const useMetaMaskInterface = () => {
  ...
  const getParentToken = async (address: any) => {
    ...
    const target = parentMinteds.filter(
      (parent: any) => parent.owner === address,
    )
    if (target.length) {
    ...
    if (chainId != cid) {
      try {
        ...
        if ((error as any).code === 4902) {
          ...
          console.error(
            'An error occurred while attempting to switch MetaMask networks',
          )
        }
      }
    }
  ...
  const mintParent = async () => {
    if (metaMaskStatus === 'connected') {
      ...
      const storefrontContract = new ethers.Contract(
        config.addresses.storefrontAddress,
        storefrontABI,
        metaMaskSigner,
      )

      try {
        // const id = await getParentToken(metaMaskAccount)
        // fetch(
        //     'http://34.28.101.209:3001/parent/'+ metaMaskAccount +'/' + id,
        //   )
        //     .then((res) => res.json())
        //     .then((out) => out)
        //     .catch((err) => {
        ...
        if (parseError(err)) {
          ...
          setMetaMaskError(
            'Unexpected error occurred while minting. Please try again.',
          )
        }
        setMetaMaskFocus(false)
      }
