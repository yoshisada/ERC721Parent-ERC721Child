...
export class URIUpdated extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save URIUpdated entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type URIUpdated must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("URIUpdated", id.toString(), this);
    }
  }

  static load(id: string): URIUpdated | null {
    return changetype<URIUpdated | null>(store.get("URIUpdated", id));
  }
