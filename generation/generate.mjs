...
import zim, { Container, Rectangle, Triangle } from "./scripts/zim.mjs";
...
function getCurve(frame, address, extraSeed="") {
  ...
  if (extraSeed != "") {
    console.log("Drawing extra");
    //drawSquares(stageW, stageH, extraSeed);
    //drawTriangles(stageW, stageH, extraSeed);
  ...
  */
  ...
  for (let i=0; i < numSquares/2; i++) {
    for (let j=0; j < numSquares/2; j++) {
      ...
      let curColor = pickColor(rng(), false);
